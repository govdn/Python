...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        if (self.is_child(a,b) or self.is_child(b,a)):
            return (-1, 1)
        elif ((Member(a).get_parent()==Member(b).get_parent()) and (Member(a).children == Member(b).children)):
            return (-1,0)
        else: 
            while( Member(self.temp).get_parent() != None):
                self.typa = self.typa + 1
                self.ancha.append(Member(self.temp).get_parent())
                self.temp = Member(self.temp).get_parent()
            while(not Member(self.temp2).get_parent()):
                self.typb += 1
                self.anchb.append(Member(self.temp2).get_parent())
                self.temp2 = Member(self.temp2).get_parent()  
            return (min(self.typa,self.typb),abs(self.typa - self.typb))
